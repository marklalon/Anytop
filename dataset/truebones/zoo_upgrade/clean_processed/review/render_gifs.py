#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Render every processed zoo_upgrade clip to a 224x224 review GIF.

The sibling gallery for ``zoo/truebones_processed`` (``../../../zoo/
truebones_processed/review/render_gifs.py``) and this one answer the same
question -- what is in the clip that sits next to it in ``bvhs/`` and
``motions/`` -- and the scene, the camera solver and the GIF packing here are
that script's, unchanged, so the two galleries read the same way.

One thing is not the same, and it is the reason this file exists: the
``zoo_upgrade`` sources are **skeletons only**.  Every GLB under
``zoo_upgrade/clean`` carries a skin and an animation and not one mesh, so
there is nothing to photograph -- an armature does not render.  So the picture
is built: one capsule per bone (a cylinder between the two joints it joins,
closed with a hemisphere at each end) and one sphere at every joint, generated
as a plain mesh at the posed joint positions of each rendered frame::

    clean_processed/review/gif/<Species>_<Action>.gif

Two decisions are worth stating.

**How thick a capsule is.**  ``RADIUS_RATIO`` of the skeleton's mean bone
length -- its own bones set its own scale, so a 13-bone Hen and an 89-bone
serpent_man are each drawn at the weight their own skeleton suggests rather
than at one library-wide number.  The mean alone is not enough at the two ends
of this library, though: measured over all 30 species the mean bone is between
3.4% (Cobra: 31 short segments strung down a long snake) and 21% (Hen: 13 bones
and nothing else) of the skeleton's own bounding diagonal, a 6x spread that at
224 pixels is the difference between a one-pixel thread and a sausage.  So the
radius is clamped into ``RADIUS_SPAN_CLAMP`` of that diagonal, a band set so the
clamp catches exactly those two species and the mean rules the other 28.

**Which segments are drawn.**  Every parent-child pair of the imported
armature, which for all 30 species is exactly the joint set ``cond.npy`` lists
in ``joints_names`` -- so the GIF shows the skeleton the model is trained on and
not a nearer-to-hand approximation of it.  Blender's own bone *tails* are not
used at all: the glTF importer synthesises them (a Bear's ``root`` comes in 119
units long, half the animal), and the joints -- the bone heads -- are the only
part of the imported rest that means anything.  Eleven of the thirty rigs put a
locomotion root on the floor under the hips, so those skeletons are drawn with a
stem running up from the ground; that stem is real dataset geometry (the clip's
root translation lives on it) and is left in.

Run with the project venv so ``bpy`` resolves::

    .venv/Scripts/python.exe Anytop/dataset/truebones/zoo_upgrade/clean_processed/review/render_gifs.py
    .venv/Scripts/python.exe .../render_gifs.py --filter Horse --overwrite
    .venv/Scripts/python.exe .../render_gifs.py --clip 'Cobra_*' -j 4 --dry-run
"""
...
PROCESSED_ROOT = os.path.dirname(HERE)                       # clean_processed
ZOO_ROOT = os.path.dirname(PROCESSED_ROOT)                   # zoo_upgrade

# -- paths -------------------------------------------------------------------
RAW_ROOT = os.path.join(ZOO_ROOT, "clean")
...
SCENE_FPS = 30           # every zoo_upgrade clip was authored at 30; this is the
#                          grid the glTF importer converts key times onto

# -- capsules ----------------------------------------------------------------
SKELETON_NAME = "ReviewSkeleton"
RADIUS_RATIO = 0.25      # capsule radius, in mean bone lengths (see the module
#                          docstring for why this alone is not enough)
RADIUS_SPAN_CLAMP = (0.012, 0.05)   # ... and its floor/ceiling, as a fraction of
#                          the rest skeleton's own bounding diagonal
...
def _load_clip_sources(metadata_path=METADATA_PATH):
    ...
    The clip stem *is* the BVH stem (``Bear_AtkRun``), which is what the GIF is
    named after.  Nothing else in the tree records which source file a clip came
    from, and the filename rules that produced it are lossy.
    """
...
def _load_facing(cond_path=COND_PATH):
    ...
    +Y turn snapped to a quarter (in this library: 0 for 26 species, +90 for
    Cobra and Rabbit2, 180 for Crow and Seagull).  glTF's +Y is Blender's +Z and
    glTF's +Z is Blender's -Y (the importer's Y-up conversion is a rotation, so a
    turn about +Y keeps its angle), and Blender's -Y is the axis
    ...
    The zoo gallery verified that against the head-vs-hips direction of all 70 of
    its measurable species; the convention is shared, so it is reused here.
...
def _load_scales(cond_path=COND_PATH):
    ...
    The GLBs are not on a shared scale: as imported, a Rabbit2 spans 0.75 world
    units and a Horse 360, because 11 of the 30 assets carry a 0.01 node scale
    and the rest carry 1.0.  The grid is what pays for that.  Its cell is a fixed
    world unit, so at the small end a whole creature sits inside one cell and at
    the large end the lines converge into flat grey and the 200-unit ground
    plane's own edge comes into frame.

    ``scale_factor`` is the number the preprocessor multiplies each species by to
    reach the canonical size the model trains on, so it is both the fix and the
    right unit: applied, the library lands between 0.8 and 2.5 units and one cell
    means the same fraction of a body everywhere.  Note it multiplies the
...
def _source_fps(times, scene_fps):
    ...
    Every zoo_upgrade clip was authored at 30 fps, so this is a constant in
...
def _skeleton_links(armature, rest):
    ...
    on top of each other in the rest pose (seven species have at least one: an
    aux joint, a mirrored pivot).  A zero-length segment has no axis to build a
    capsule around and nothing to show that its joint sphere does not; dropping
    it at *rest* rather than per frame is what keeps the mesh topology fixed for
    the whole clip, which is what lets the render just overwrite coordinates.
    """
...
def _capsule_radius(rest, links, ratio, clamp):
    """``(radius, mean bone length)`` in world units -- see the module docstring.

    A skeleton with no drawable link at all (there is none in this library, but a
    single-joint rig would be one) falls back to a tenth of its own span, which
    for a lone joint sphere is simply a visible dot.
    """
...
def _ground_anchor(points):
    ...
    ``z = 0`` and keeps its shadow while a flier shows at most a quarter of its
    own height of air beneath it instead of shrinking to a speck above a
    mandatory floor.
    """
...
def _rescale_to_canonical(bpy, canonical_scale):
    """Scale the imported skeleton onto the dataset's canonical size.
    ...
    eleven of the thirty assets, 1.0 on the rest), so the node scale is divided
    back out.  Only the roots are touched; children ride along on the parent
    transform.
...
def _load_glb(bpy, path, opts):
    """Fresh scene, GLB imported, everything that is not the armature dropped.

    The fps is set *before* the import: the glTF importer converts the clip's
    keyframe times from seconds using the scene fps, and every zoo_upgrade clip
    was authored at 30, so anything else lands the keys off the frame grid.

    The meshes deleted here are not the creature -- these files carry none.  They
    are the unit ``Icosphere`` origin marker every asset in this library ships;
    left in, it would render as a ball at the world origin.
    """
...
def _render_frames(bpy, glb_path, frame_dir, opts):
    ...
    scene, armature, sun = _load_glb(bpy, glb_path, opts)
    if armature is None:
    ...
    radius, _mean = _capsule_radius(rest, links, opts["radius_ratio"],
                                    opts["radius_clamp"])
    skeleton = _SkeletonMesh(bpy, scene, links, len(rest), radius,
                             opts["joint_scale"])

    # Every frame's geometry up front: the camera is solved over the whole clip,
    # so the clouds have to exist before the first frame is rendered anyway, and
    # holding them means each frame is posed once rather than twice.
    clouds = []
    ...
    paths = []
    with _quiet_fds():
        for index, (cloud, position) in enumerate(zip(clouds, positions), 1):
            skeleton.pose(cloud)
...
def _run_job(job):
    ...
    frame_dir = os.path.join(keep, clip) if keep else tempfile.mkdtemp(prefix="zugif_")
    try:
...
# --- worker stdout plumbing (mirrors the zoo gallery) ----------------------
...
def main():
    ap = argparse.ArgumentParser(
        description="Render every processed zoo_upgrade clip to a review GIF.")
    ap.add_argument("--filter", default="*",
                    help="species glob, e.g. 'Horse*' (default: %(default)s)")
    ...
    ap.add_argument("--radius-ratio", type=float, default=RADIUS_RATIO,
                    help="capsule radius in mean bone lengths "
                         "(default: %(default)s)")
    ap.add_argument("--radius-clamp", type=float, nargs=2,
                    default=list(RADIUS_SPAN_CLAMP), metavar=("MIN", "MAX"),
                    help="floor and ceiling on that radius, as a fraction of the "
                         "rest skeleton's bounding diagonal (default: %(default)s)")
    ap.add_argument("--joint-scale", type=float, default=JOINT_SCALE,
                    help="joint sphere radius in capsule radii; 1.0 draws plain "
                         "capsules with no ball at the joint (default: %(default)s)")
    ...
    ap.add_argument("--no-facing", dest="facing", action="store_false",
                    help="ignore cond.npy's orientation_quat; the four species "
                         "that need it then render side-on or from behind")
    ap.add_argument("--no-canonical-scale", dest="canonical_scale",
                    action="store_false",
                    help="import the GLBs at their own scale; the grid cell then "
                         "means a different fraction of a body per species")
    ...
    print("engine %s  %dpx (x%d)  %d colors  dither %s  %.0f fps  <=%d frames  "
          "follow %s  radius %.3f x mean bone, clamped to [%.3f, %.3f] of span, "
          "joints x%.2f"
          % (args.engine, args.size, args.supersample, args.colors, args.dither,
             args.fps, args.max_frames, args.follow, args.radius_ratio,
             args.radius_clamp[0], args.radius_clamp[1], args.joint_scale),
          flush=True)
