...
def get_general_skeleton_3d_motion(parents, joints, title, dataset, figsize=(7, 7), fps=120, radius=5, face_joints = [], fc = None):
    ...
    global_mins = data.min(axis=(0, 1))
    global_maxs = data.max(axis=(0, 1))
    center = (global_mins + global_maxs) / 2.0
    half_extent = np.max(global_maxs - global_mins) / 2.0
    half_extent = max(float(half_extent * 1.2), 0.75)
    ...
    def update(index):
        ...
        ax.view_init(elev=120, azim=-90)
        ax.dist = 7.5
        ax.set_xlim3d([center[0] - half_extent, center[0] + half_extent])
        ax.set_ylim3d([max(0.0, global_mins[1] - 0.05), max(center[1] + half_extent, global_maxs[1] + 0.05)])
        ax.set_zlim3d([center[2] - half_extent, center[2] + half_extent])
        fig.suptitle(title, fontsize=10)
        for joint, parent in enumerate(parents[1:], start=1):
            ax.plot3D(
                data[index, [joint, parent], 0],
