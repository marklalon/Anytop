...
def truebones_batch_collate(batch):
    ...
    for b in batch:  
        ...
        for extra in b[14:]:
            if isinstance(extra, dict):
                if 'joint_mask_candidate_roots' in extra:
                    extra_cond = extra
                elif any(key in extra for key in ('action_category', 'species_label', 'action_tags', 'translation_root_index', 'is_loop', 'loop_full_cycle')):
        ...
        if motion_metadata is not None:
            for key in ('species_label', 'species_group', 'action_tags', 'translation_root_index', 'is_loop', 'loop_full_cycle'):
