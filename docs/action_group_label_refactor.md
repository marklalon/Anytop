...
| 职责 | 入口 | 现状 |
|---|---|---|
| 训练集切分 | `--action_tags getup,death,fall,rest,jump,turn,gethurt`（改造前的 `train.bat`，该脚本先被按 action group 拆分的 `train_<group>.bat` 取代，2026-09-20 起又合回单一的 `train_all.bat`，见 [unified_action_group_training.md](unified_action_group_training.md)） | 手动列出某一组的全部 tag |
...
| [train_all.bat](../train_all.bat)（原 `train_<group>.bat` 三个脚本，已合并）、[multi_dataset_training.md](./multi_dataset_training.md)、README | 参数与训练契约描述 |
