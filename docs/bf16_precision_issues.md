...
在一次训练步性能实验中，链式矩阵乘的结合顺序发生变化。结果是 loss 对模型输出的梯度在 115.2 万个元素里只有 13 个发生变化，最大差 1.5e-8（梯度最大值是 1.3e-4）。
...
- **记录**：
  1. `l_simple`、`geodesic_loss`、`loop_*` 指标曲线（按物种族拆分的 `l_simple_*` 已于 2026-09-14 移除）；
