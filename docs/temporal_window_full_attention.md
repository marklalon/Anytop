...
> 这 9 项是当时**显式**带 `--loop` 的任务。`--loop` 现在默认 `auto`（有 reference 时按 reference 的
> 闭合判决，否则按该 label 训练片段里 loop 的多数），电池里未带该 flag 的任务也有一部分会解析为
> 闭合窗口，所以"带 `--loop`"已不再等同于"按 loop 生成"。上表是当时的实测记录。
...
> 表中“不带 `--loop`”是当时的语义（省略即开放窗口）。`--loop` 现在的默认值是 `auto`，省略时按
> reference / 语料解析，复现这张表要显式写 `--loop off`。
