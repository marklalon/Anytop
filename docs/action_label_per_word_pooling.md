...
> 以几何预检报告与契约模块为准，本文不固化，避免随代码变动而失效。
>
> **三条长期结论，先说清楚**：
...
| hands | `hand0` / `hand1` / `hand2`（至多一个） | 通道 = 该词向量本身 |
...
- **hands 轴**：`hand0` / `hand1` / `hand2` 互斥、至多一个；**不写 = 未指定**（该物种的边缘分布），
  `hand0` 是肯定陈述（「空手」）不是默认值。语义是手部**占用数**，不是武器类别：剑 + 盾 = hand2（与步枪
...
- **两层指纹 gate load / resume / checkpoint-bind**：
  ...
  - 改槽规则只失效后者；改 T5 文本 / EOS / 后处理两者都失效。当前具体值见几何预检报告。

## 5. 几何预检（原则）

只对模型**改不回来**的性质设硬门；各向异性指标（p95 / 最近邻中位 / 有效秩）只报告、不阻断。

- **硬门**：不同标签余弦碰撞 = 0；追加修饰词后 head / direction 逐位不变；每槽源满秩；
  `latent_dim` ≥ 各槽源总秩；`(group, {word})` 键唯一。
- **为什么这些是硬门**：槽源满秩意味着不同成员集合的归一化和不可能相同、且存在线性 readout 能判断每个词
  是否在槽内，覆盖解析器允许的**全部**非空槽组合（含上限 8 词），不只覆盖语料见过的组合。
- 当前满秩值、最坏近邻、通道漂移等实测数字见预检工具的输出，本文不固化。

## 6. 决策历史（只留「为什么」和不变量，去掉实现细节）

### hands 轴：`weapon` + `1hand/2hand` → `hand0 / hand1 / hand2`

旧轴没有「空手」这个值，且标注不一致（combat idle 写 weapon、attack / hurt / die 不写），「没写 weapon 的
clip」其实是持械与空手的混合。改成互斥占用数 + 独立第四槽：单独成槽是因为 hands 词会出现在有手物种几乎
每条 clip 上，塞进 modifier 会让每个真修饰词的份额减半（`attack, slash, hand2` 里 slash 被稀释），且
「带不带 hand 词」读数不同。独立槽下其它三通道逐位相同，模型只需分开三个点。
