...
TEST_T5_DIM = 768
# At least the total slot source rank (the slots partition the vocabulary, so
# the ranks add to its size), which model construction refuses to go under:
# below it the first Linear cannot separate every label. The width sits at or
# above that rank and is one the test models' attention can split evenly
# (num_heads=2).
TEST_LATENT_DIM = 140
